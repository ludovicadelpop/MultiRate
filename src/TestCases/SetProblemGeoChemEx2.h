/*
 * SetProblemGeoChemEx2.h
 *
 *  Created on: Mar 16, 2016
 *      Author: delpopol
 */

#ifndef SETPROBLEMGEOCHEMEX2_H_
#define SRC_TESTCASES_SETPROBLEMGEOCHEMEX2_H_
...
namespace SetProblemGeoChemEx2{
	GeochemistryParameters getParameters(RealVector& y0){
		    std::cout << "Get Parameters for extented TestSHPCO2 " << "\n" ;
		    ...
		    for(Integer i=0; i<100; i++){
		    	...
		    	i=i+1;
		    	String tmp_name2 = "sp"+std::to_string(i);
		    	ChemicalSpecies species2(tmp_name2,"w","Solute","Ideal",2.0861861);
		    	system.addSpecies(species2);
		    }
			...
			String tmp_name = "sr"+std::to_string(i);
			ChemicalReaction reactionR(tmp_name,1.e-1);
				Integer j=i*2;
				String tmp_name_sp = "sp"+std::to_string(j);
				KeyReal sp1(tmp_name_sp ,1);
				reactionR.addSpecies(sp1);
				j=j+1;
				String tmp_name_sp2 = "sp"+std::to_string(j);
				KeyReal sp2(tmp_name_sp2 ,1);
				reactionR.addSpecies(sp2);
				j=j+1;
				String tmp_name_sp3 = "sp"+std::to_string(j);
				KeyReal sp3(tmp_name_sp3 ,-1);
				reactionR.addSpecies(sp3);
				j=j+1;
				String tmp_name_sp4 = "sp"+std::to_string(j);
				KeyReal sp4(tmp_name_sp4 ,-1);
				reactionR.addSpecies(sp4);
			system.addReaction(reactionR);
			}

			ChemicalReaction reactionR("sr49",1.e-1);
			KeyReal sp160("sp98",1);
			reactionR.addSpecies(sp160);
			KeyReal sp170("sp99",1);
			reactionR.addSpecies(sp170);
			KeyReal sp180("sp0",-1);
			reactionR.addSpecies(sp180);
			KeyReal sp190("sp1",-1);
			reactionR.addSpecies(sp190);
			system.addReaction(reactionR);
